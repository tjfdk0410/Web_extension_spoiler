...
def predict():
    ...
    if flask.request.method == "POST":
        if flask.request.files.get("reviews"):
            reviews = flask.request.files["reviews"].read()
            #예측한 결과를 prds에 넣는다
            #리스트 형태로 어떻게 변환?!?!?!?!?!?!?!?
            preds = predict_text(reviews)
            for t in preds:
                #각각의 결과는 배열 안에 있으므로 0index를 참조한다. ex) preds = [[0],[1],[1]]
                data["predictions"].append(t[0])
            data["success"] = True
